...
<Bounded data-slice-type={slice.slice_type} data-slice-variation={slice.variation} class="relative">
	...
	<div class="showcase__glow absolute -z-10 aspect-video w-full max-w-2xl rounded-full bg-violet-500 mix-blend-screen blur-[120px] filter"/>
	...
	{/if}
	<div class="relative mt-16 grid items-center gap-8 rounded-xl border border-violet-50/20 
	bg-gradient-to-b from gray-50/15 to-gray-50/5 px-8 
	...
	<div class="grid-background"/>
		...
		<div>
			...
			{#if slice.primary.button_link}
				<ButtonLink field={slice.primary.button_link} class="mt-6">{slice.primary.button_text || "Learn more"}</ButtonLink>
			{/if}
		</div>

		<PrismicImage field={slice.primary.image} class={clsx(
