<script>
	...
	import { PrismicRichText, PrismicText, PrismicLink, PrismicImage } from '@prismicio/svelte'
...
<Bounded data-slice-type={slice.slice_type} data-slice-variation={slice.variation}>
	...
	<div class="mt-20 grid gap-16">
		{#each caseStudies as caseStudy, index}
			<div class="relative group grid gap-4 opacity-85 transition-opacity 
			duration-300 hover:cursor-pointer
			 ...
			 >
				...
				<div class={clsx("relative lg:col-span-2", index % 2 && "md:-order-1")}>
					...
					<div class="image-glow bg-violet-500 -top-8 -right-4"></div>
					<PrismicImage 
...
<style>
	.image-glow {
		@apply absolute h-1/2 w-1/2 rounded-full opacity-0 
		mix-blend-screen blur-3xl transition-opacity duration-500 group-hover:opacity-30
	}
</style>